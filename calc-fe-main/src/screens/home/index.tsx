...
// //             answer = String(answer); // Convert to string
// //           }
...
//                     ))}
//                 </Group>
//                 <Button onClick={() => setIsEraser(!isEraser)} className='z-20 bg-black text-white'>
//                     {isEraser ? 'Draw' : 'Erase'}
//                 </Button>
//                 <Button onClick={runRoute} className='z-20 bg-black text-white'>Run</Button>
//             </div>
//             <canvas ref={canvasRef} className='absolute top-0 left-0 w-full h-full' onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseOut={stopDrawing} />
...
export default function Home() {
    ...
    const [dictOfVars, setDictOfVars] = useState({});
    const [result, setResult] = useState<GeneratedResult>();
    const [latexPosition, setLatexPosition] = useState({ x: 50, y: 150 }); // Adjusted position
    const [latexExpression, setLatexExpression] = useState<Array<string>>([]);
    const [isEraser, setIsEraser] = useState(false);
    ...
    const runRoute = async () => {
        const canvas = canvasRef.current;
        if (canvas) {
            // Resize the canvas drawing
            const resizedImage = resizeCanvas(canvas, 300, 150);

            const response = await axios.post(`${import.meta.env.VITE_API_URL}/calculate`, {
                image: resizedImage, // Send resized image
                dict_of_vars: dictOfVars
            });
            ...
            setLatexPosition({ x: 20, y: 150 }); // Move output to the left
            resp.data.forEach((data: Response) => {
                setTimeout(() => {
                    setResult({ expression: data.expr, answer: data.result });
                }, 1000);
    ...
    const resizeCanvas = (canvas: HTMLCanvasElement, newWidth: number, newHeight: number) => {
        ...
        return tempCanvas.toDataURL('image/png'); // Return resized image data URL
    };
